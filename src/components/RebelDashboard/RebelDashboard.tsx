...
const SEV_COLOR: Record<string, string> = { CRITICAL: "#ef4444", HIGH: "#f97316", MEDIUM: "#eab308", LOW: "#22d3ee", NORMAL: "#22c55e" };
...
function ScanModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 300, display: "flex", alignItems: mobile ? "flex-end" : "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
      <div style={{ width: mobile ? "100%" : 560, maxHeight: mobile ? "92vh" : "90vh", background: "#0d1117", border: "1px solid rgba(59,130,246,0.25)", borderRadius: mobile ? "12px 12px 0 0" : 4, boxShadow: "0 0 60px rgba(59,130,246,0.15)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12, overflowY: "auto", flex: 1 }}>
          <div style={{ display: "flex", borderBottom: "1px solid rgba(59,130,246,0.08)" }}>
            {[["url", "URL SCAN"], ["crypto", "CRYPTO / TLS"]].map(([id, label]) => (
              <button key={id} onClick={() => setTab(id)} style={{ ...S.tabBtn, color: tab === id ? "#3b82f6" : "rgba(200,220,255,0.3)", borderBottom: tab === id ? "2px solid #3b82f6" : "2px solid transparent", flex: 1 }}>{label}</button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === "Enter" && runScan()} placeholder="https://target.com" style={{ ...S.input, flex: 1, fontSize: 14 }} />
            <button onClick={runScan} disabled={loading} style={{ ...S.btn, padding: "10px 16px" }}>{loading ? "..." : "SCAN"}</button>
          </div>
          ...
          {result && !result.error && (
            <div style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(59,130,246,0.1)", borderRadius: 3, padding: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                  {([["TLS Version", result.tls_info.tls_version], ["Key Type", result.tls_info.key_type], ["Key Size", (result.tls_info.key_size ?? "?") + " bits"], ["Issuer", result.tls_info.issuer], ["Post-Quantum", result.tls_info.post_quantum ? "YES ✓" : "NO"], ["Self-Signed", result.tls_info.is_self_signed ? "YES ⚠" : "NO"]] as [string, string | number | undefined][]).map(([k, v]) => (
                    <div key={k} style={{ background: "rgba(255,255,255,0.03)", padding: "6px 10px", borderRadius: 2 }}>
                      <div style={{ fontSize: 8, color: "rgba(200,220,255,0.28)", letterSpacing: "0.1em", marginBottom: 2 }}>{k}</div>
                      <div style={{ fontSize: 12, color: "rgba(200,220,255,0.8)" }}>{v ?? "N/A"}</div>
                    </div>
                  ))}
                </div>
              )}
              {result.vulnerabilities && (
                <div>
                  <div style={{ fontSize: 9, color: "rgba(200,220,255,0.28)", letterSpacing: "0.15em", marginBottom: 6 }}>VULNERABILITIES</div>
              ...
              {result.explanation && <div style={{ fontSize: 12, color: "rgba(200,220,255,0.58)", lineHeight: 1.7, borderTop: "1px solid rgba(59,130,246,0.08)", paddingTop: 10, fontFamily: "Share Tech Mono" }}>{result.explanation}</div>}
            </div>
          )}
...
function IPEnrichModal({ ip: initIp, onClose }: { ip: string; onClose: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 300, display: "flex", alignItems: mobile ? "flex-end" : "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
      <div style={{ width: mobile ? "100%" : 500, maxHeight: mobile ? "92vh" : "90vh", background: "#0d1117", border: "1px solid rgba(59,130,246,0.25)", borderRadius: mobile ? "12px 12px 0 0" : 4, display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 0 60px rgba(59,130,246,0.15)" }}>
        <div style={{ padding: "13px 18px", borderBottom: "1px solid rgba(59,130,246,0.1)", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        ...
        <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12, overflowY: "auto", flex: 1 }}>
          <div style={{ display: "flex", gap: 8 }}>
            <input value={ip} onChange={e => setIp(e.target.value)} onKeyDown={e => e.key === "Enter" && run()} placeholder="x.x.x.x" style={{ ...S.input, flex: 1, fontSize: 14 }} />
            <button onClick={() => run()} disabled={loading} style={{ ...S.btn, padding: "10px 16px" }}>{loading ? "..." : "ENRICH"}</button>
          </div>
          {loading && <div style={{ display: "flex", gap: 6, alignItems: "center" }}><div style={S.spinner} /><span style={{ fontSize: 11, color: "rgba(200,220,255,0.4)", fontFamily: "Share Tech Mono" }}>Querying threat feeds...</span></div>}
          {result && !result.error && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div>
                  <div style={{ fontSize: 12, color: "rgba(200,220,255,0.5)" }}>{result.action}</div>
                  <div style={{ fontSize: 11, color: "rgba(200,220,255,0.35)" }}>Confidence: {result.confidence}</div>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                {([["Country", result.intel?.country], ["ASN", result.intel?.asn], ["Org", result.intel?.org], ["Anomaly", result.base_anomaly], ["Malicious", result.intel?.malicious], ["Suspicious", result.intel?.suspicious], ["Reputation", result.intel?.reputation], ["Financial Risk", result.financialRisk]] as [string, string | number | undefined][]).map(([k, v]) => (
                  <div key={k} style={{ background: "rgba(255,255,255,0.03)", padding: "6px 10px", borderRadius: 2 }}>
                    <div style={{ fontSize: 8, color: "rgba(200,220,255,0.28)", letterSpacing: "0.1em", marginBottom: 2 }}>{k}</div>
                    <div style={{ fontSize: 12, color: "rgba(200,220,255,0.8)" }}>{v ?? "-"}</div>
                  </div>
                ))}
              </div>
              <button onClick={doBlock} disabled={blocked} style={{ ...S.btn, background: blocked ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.12)", borderColor: blocked ? "rgba(34,197,94,0.4)" : "rgba(239,68,68,0.35)", color: blocked ? "#22c55e" : "#ef4444", padding: "12px 0", width: "100%", fontSize: 11 }}>
...
function ChatPanel({ onClose }: { onClose: () => void }) {
  ...
  if (mobile) {
    return (
      <div style={{ position: "fixed", inset: 0, background: "#0a0f1a", zIndex: 200, display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "14px 18px", borderBottom: "1px solid rgba(59,130,246,0.12)", display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "max(14px, env(safe-area-inset-top))" }}>
          ...
          <button onClick={onClose} style={{ ...S.ghost, fontSize: 22, padding: 8 }}>✕</button>
        </div>
        ...
        <div style={{ padding: "12px 14px", paddingBottom: "max(12px, env(safe-area-inset-bottom))", borderTop: "1px solid rgba(59,130,246,0.12)", display: "flex", gap: 10, background: "#0a0f1a" }}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Query REBEL..." style={{ ...S.input, flex: 1, fontSize: 16, padding: "12px 14px" }} />
          <button onClick={send} disabled={loading} style={{ ...S.btn, padding: "12px 18px", fontSize: 10 }}>SEND</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
      <svg width="72" height="72" viewBox="0 0 100 100">
        ...
        <circle cx="50" cy="50" r={r} fill="none" stroke={color} strokeWidth="8" strokeDasharray={`${dash} ${circ}`} strokeLinecap="round" transform="rotate(-90 50 50)" style={{ transition: "stroke-dasharray 1s ease", filter: `drop-shadow(0 0 5px ${color})` }} />
        <text x="50" y="54" textAnchor="middle" fill={color} fontSize="16" fontFamily="Orbitron" fontWeight="700">{score}</text>
      </svg>
      <span style={{ fontSize: 8, color: "rgba(200,220,255,0.38)", letterSpacing: "0.08em", fontFamily: "Share Tech Mono", textTransform: "uppercase" }}>{label}</span>
    </div>
  );
}
...
function MobileNav({ onChat, onScan, chatOpen }: { onChat: () => void; onScan: () => void; chatOpen: boolean }) {
  return (
    <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "rgba(8,12,20,0.98)", borderTop: "1px solid rgba(59,130,246,0.12)", display: "flex", zIndex: 150, paddingBottom: "env(safe-area-inset-bottom)" }}>
      {[
        ...
        <button key={item.label} onClick={item.action} style={{ flex: 1, background: "none", border: "none", padding: "12px 0", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer", borderTop: item.active ? "2px solid #3b82f6" : "2px solid transparent" }}>
          <span style={{ fontSize: 18, color: item.active ? "#3b82f6" : "rgba(200,220,255,0.4)" }}>{item.icon}</span>
          <span style={{ fontFamily: "'Orbitron',monospace", fontSize: 8, color: item.active ? "#3b82f6" : "rgba(200,220,255,0.35)", letterSpacing: "0.15em" }}>{item.label}</span>
        </button>
      ))}
    </div>
  );
...
export default function RebelDashboard() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080c14", fontFamily: "Share Tech Mono", color: "rgba(200,220,255,0.85)", backgroundImage: "radial-gradient(ellipse at 15% 50%, rgba(59,130,246,0.03) 0%, transparent 55%)" }}>
      <style>{`
      ...
      <nav style={{ height: mobile ? 48 : 52, display: "flex", alignItems: "center", justifyContent: "space-between", padding: mobile ? "0 14px" : "0 22px", paddingTop: mobile ? "env(safe-area-inset-top)" : 0, borderBottom: "1px solid rgba(59,130,246,0.09)", background: "rgba(8,12,20,0.97)", backdropFilter: "blur(10px)", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: mobile ? 8 : 12 }}>
          ...
          <span style={{ fontFamily: "'Orbitron',monospace", fontWeight: 900, fontSize: mobile ? 15 : 16, color: "#fff", letterSpacing: "0.22em" }}>REBEL</span>
          {!mobile && <><div style={{ width: 1, height: 18, background: "rgba(59,130,246,0.18)" }} /><span style={{ fontSize: 8, color: "rgba(200,220,255,0.27)", letterSpacing: "0.14em" }}>THREAT INTELLIGENCE PLATFORM</span></>}
        </div>
        {!mobile && (
      ...
      <div style={{ padding: mobile ? "12px 12px 80px" : "16px 22px", display: "flex", flexDirection: "column", gap: mobile ? 10 : 14, marginRight: (!mobile && chatOpen) ? 400 : 0, transition: "margin-right 0.3s ease" }}>
        ...
        {/* METRICS — 2x3 on mobile, 5 across on desktop */}
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(5,1fr)", gap: mobile ? 8 : 9 }}>
          {[
            ...
            <div key={i} className="panel" style={{ padding: mobile ? "10px 12px" : "12px 14px" }}>
              ...
              <div style={{ fontSize: 8, color: "rgba(200,220,255,0.25)", letterSpacing: "0.06em" }}>{m.sub}</div>
            </div>
          ))}
        </div>
          ...
          <div style={{ padding: "10px 4px 4px" }}>
            <ResponsiveContainer width="100%" height={mobile ? 120 : 160}>
              <AreaChart data={trafficHistory}>
                ...
                <YAxis tick={{ fontSize: 8, fill: "rgba(200,220,255,0.22)", fontFamily: "Share Tech Mono" }} axisLine={false} tickLine={false} width={28} />
        ...
        {/* RISK ARCS — horizontal scroll on mobile */}
        <div className="panel">
          <div className="ph">
        ...
        <div className="panel">
          ...
          {mobile ? (
            ...
            <div style={{ maxHeight: 280, overflowY: "auto" }}>
              {displayFeed.map((p, i) => (
                <div key={p.id} className="row fr" onClick={() => setEnrichTarget(p.src)} style={{ padding: "10px 14px", borderBottom: "1px solid rgba(59,130,246,0.04)", background: i === 0 && p.label === "CRITICAL" ? "rgba(239,68,68,0.04)" : "transparent" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "rgba(200,220,255,0.7)", fontSize: 12 }}>{p.src}</span>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <div style={{ width: 40, height: 3, background: "rgba(255,255,255,0.07)", borderRadius: 2, overflow: "hidden" }}>
                        <div style={{ width: `${p.risk * 100}%`, height: "100%", background: p.color }} />
                      </div>
                      <span style={{ fontSize: 10, color: p.color }}>{(p.risk * 100).toFixed(0)}%</span>
                    </div>
                  </div>
                  <div style={{ marginTop: 3, fontSize: 10, color: "rgba(200,220,255,0.35)" }}>:{p.dst_port} · {p.size}B · {p.country}</div>
                </div>
              ))}
              {displayFeed.length === 0 && <div style={{ padding: 18, textAlign: "center", fontSize: 11, color: "rgba(200,220,255,0.18)" }}>Awaiting packets...</div>}
            </div>
          ) : (
            // Desktop: table-style
            <>
              ...
              <div style={{ maxHeight: 268, overflowY: "auto" }}>
                ...
                {displayFeed.length === 0 && <div style={{ padding: 18, textAlign: "center", fontSize: 10, color: "rgba(200,220,255,0.18)" }}>Awaiting packets...</div>}
              </div>
            </>
          )}
        </div>
        ...
        <div className="panel">
          ...
          <div style={{ maxHeight: mobile ? 200 : 180, overflowY: "auto" }}>
            {flaggedIPs.length === 0 && <div style={{ padding: 14, fontSize: 11, color: "rgba(200,220,255,0.18)" }}>No flagged sources yet...</div>}
            {flaggedIPs.map(p => (
              <div key={p.src} className="row" onClick={() => setEnrichTarget(p.src)} style={{ display: "flex", alignItems: "center", gap: 8, padding: mobile ? "11px 14px" : "8px 13px", borderBottom: "1px solid rgba(59,130,246,0.04)" }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: p.color, boxShadow: `0 0 4px ${p.color}`, flexShrink: 0 }} />
                <span style={{ flex: 1, fontSize: mobile ? 13 : 11, color: "rgba(200,220,255,0.68)" }}>{p.src}</span>
                <span style={{ fontSize: 10, color: "rgba(200,220,255,0.28)" }}>{p.country}</span>
                <span style={{ fontSize: 9, fontFamily: "'Orbitron',monospace", color: p.color }}>{p.label}</span>
                <span style={{ fontSize: 10, color: "rgba(200,220,255,0.2)" }}>→</span>
              </div>
            ))}
          </div>
        </div>

        {/* BACKEND SERVICES — hidden on mobile to save space */}
        {!mobile && (
          <div className="panel">
        ...
        <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderTop: "1px solid rgba(59,130,246,0.06)" }}>
          <span style={{ fontSize: 7, color: "rgba(200,220,255,0.14)", letterSpacing: "0.1em" }}>REBEL — RESTRICTED ACCESS</span>
          <span style={{ fontSize: 7, color: "rgba(200,220,255,0.14)", letterSpacing: "0.1em" }}>{API.replace("https://", "")}</span>
        </div>
      </div>
