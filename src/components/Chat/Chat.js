        ...
        <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {result && !result.error && (
            <div style={{ background:"rgba(255,255,255,0.025)", border:"1px solid rgba(59,130,246,0.1)", borderRadius:3, padding:14, display:"flex", flexDirection:"column", gap:10 }}>
              ...
              {result.tls_info && result.tls_info.tls_version && (
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
                  {[["TLS Version",result.tls_info.tls_version],["Key Type",result.tls_info.key_type],["Key Size",(result.tls_info.key_size||"?")+(" bits")],["Issuer",result.tls_info.issuer],["Post-Quantum",result.tls_info.post_quantum?"YES ✓":"NO"],["Self-Signed",result.tls_info.is_self_signed?"YES ⚠":"NO"]].map(([k,v])=>(
              ...
              {result.vulnerabilities && (
                <div>
                  ...
                  {result.vulnerabilities.missing_headers?.map(h=><div key={h} style={{fontSize:10,color:"#f97316"}}>⚠ {h}</div>)}
                  {result.vulnerabilities.open_redirect_risk&&<div style={{fontSize:10,color:"#ef4444"}}>⚠ Open Redirect Risk</div>}
                  {result.vulnerabilities.suspicious_url_patterns?.map(p=><div key={p} style={{fontSize:10,color:"#eab308"}}>⚠ {p}</div>)}
                </div>
              )}
              {result.reasons?.map(r=><div key={r} style={{fontSize:10,color:"#f97316"}}>⚠ {r}</div>)}
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}>
      <div style={{width:500,background:"#0d1117",border:"1px solid rgba(59,130,246,0.25)",borderRadius:4,boxShadow:"0 0 60px rgba(59,130,246,0.15)"}}>
        <div style={{padding:"13px 18px",borderBottom:"1px solid rgba(59,130,246,0.1)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={S.label}>IP INTELLIGENCE — {API.replace("https://","")}/enrich_ip</span>
          <button onClick={onClose} style={S.ghost}>✕</button>
        </div>
        <div style={{padding:18,display:"flex",flexDirection:"column",gap:12}}>
          <div style={{display:"flex",gap:8}}>
          ...
          {result && !result.error && (
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
                {[["Country",result.intel?.country],["ASN",result.intel?.asn],["Org",result.intel?.org],["Anomaly Score",result.base_anomaly],["Malicious Signals",result.intel?.malicious],["Suspicious Signals",result.intel?.suspicious],["Reputation Score",result.intel?.reputation],["Financial Risk",result.financialRisk]].map(([k,v])=>(
                  <div key={k} style={{background:"rgba(255,255,255,0.03)",padding:"5px 9px",borderRadius:2}}>
                    <div style={{fontSize:7,color:"rgba(200,220,255,0.28)",letterSpacing:"0.12em",marginBottom:2}}>{k}</div>
...
function ChatPanel({ onClose }) {
  const [msgs, setMsgs] = useState([{role:"assistant",text:"REBEL ONLINE. Direct line to threat intelligence core. Query active."}]);
  ...
  return (
    <div style={{position:"fixed",right:0,top:0,bottom:0,width:400,background:"#0a0f1a",borderLeft:"1px solid rgba(59,130,246,0.18)",display:"flex",flexDirection:"column",zIndex:200,animation:"slideIn 0.28s ease",boxShadow:"-18px 0 50px rgba(0,0,0,0.55)"}}>
      <div style={{padding:"13px 18px",borderBottom:"1px solid rgba(59,130,246,0.1)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}><Pulse color="#3b82f6"/><span style={S.label}>REBEL CHAT — {API.replace("https://","")}/chat</span></div>
        <button onClick={onClose} style={S.ghost}>✕</button>
      </div>
      ...
      <div style={{padding:"11px 13px",borderTop:"1px solid rgba(59,130,246,0.1)",display:"flex",gap:8}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Query REBEL intelligence..." style={{...S.input,flex:1}}/>
...
function Pulse({color="#3b82f6"}) {
  return (
    <span style={{position:"relative",display:"inline-flex",width:8,height:8,flexShrink:0}}>
      <span style={{position:"absolute",inset:0,borderRadius:"50%",background:color,opacity:.5,animation:"ping 1.4s ease infinite"}}/>
  ...
  const fetchPacket = useCallback(async () => {
    try {
      ...
      const row = {
        ...
        src: pkt.ip_src, dst_port: pkt.dst_port,
        size: pkt.packet_size, risk, label, color,
      ...
      setTrafficHistory(prev => {
        const next = [...prev.slice(1), { t: prev[prev.length-1].t+1, normal: label==="NORMAL"?pkt.packet_size:0, anomaly: label!=="NORMAL"?pkt.packet_size:0 }];
        return next;
      });
      setStats(prev => ({
        total: prev.total+1,
    ...
    <div style={{minHeight:"100vh",background:"#080c14",fontFamily:"Share Tech Mono",color:"rgba(200,220,255,0.85)",backgroundImage:"radial-gradient(ellipse at 15% 50%, rgba(59,130,246,0.03) 0%, transparent 55%), radial-gradient(ellipse at 85% 15%, rgba(37,99,235,0.045) 0%, transparent 50%)"}}>
      ...
      <div style={{padding:"16px 22px",display:"flex",flexDirection:"column",gap:14,marginRight:chatOpen?400:0,transition:"margin-right 0.3s ease"}}>
          ...
          {[
            ...
            <div key={i} className="panel" style={{padding:"12px 14px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                ...
                <span style={{fontSize:12,color:m.color,opacity:.65}}>{m.icon}</span>
              </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 360px",gap:12}}>
          <div className="panel">
            <div className="ph">
              <div style={{display:"flex",alignItems:"center",gap:8}}><Pulse color="#3b82f6"/><span style={S.label}>LIVE TRAFFIC — ANOMALY DETECTION · /latest_packet</span></div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderTop:"1px solid rgba(59,130,246,0.06)"}}>
          <span style={{fontSize:7.5,color:"rgba(200,220,255,0.16)",letterSpacing:"0.13em"}}>REBEL SECURITY INTELLIGENCE PLATFORM — RESTRICTED ACCESS — {new Date().toUTCString()}</span>
