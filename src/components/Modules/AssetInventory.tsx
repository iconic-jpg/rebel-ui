  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display:"grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: mobile ? 8 : 10 }}>
        ...
        <Panel>
          <PanelHeader
            left="ASSET RISK DISTRIBUTION"
            right={<span style={{ fontFamily:"'Orbitron',monospace", fontSize:20, color:T.red }}>
              {riskCounts.Critical + riskCounts.High}<span style={{fontSize:12}}> high</span>
            </span>}
          />
            ...
            {filtered.map((a, i) => (
              <div key={i} style={{ padding:"10px 14px", borderBottom:`1px solid rgba(59,130,246,0.05)` }}>
                ...
                <div style={{ fontSize:10, color:"rgba(59,130,246,0.6)", marginBottom:4,
                  overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{a.url}</div>
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  <Badge v="gray">{a.type}</Badge>
                  <Badge v={certVariant(a.cert)}>{a.cert}</Badge>
                  <span style={{ fontSize:9, color:T.text3 }}>{a.scan}</span>
                </div>
              </div>
            ))}
            {filtered.length === 0 && (
              <div style={{ padding:14, fontSize:10, color:T.text3 }}>No assets found</div>
            )}
          </div>
        ) : (
          // Desktop table view
            ...
            {[
              ...
              <div key={row.label} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                <div style={{ width:8, height:8, borderRadius:"50%", background:row.color,
                  boxShadow:`0 0 4px ${row.color}`, flexShrink:0 }} />
                <div style={{ flex:1 }}>
        ...
        <Panel>
          ...
          {mobile ? (
            ...
            <div style={{ maxHeight:220, overflowY:"auto" }}>
              {assets.filter(a => a.tls !== "—" && a.tls !== undefined).slice(0, 10).map((a, i) => (
                <div key={i} style={{ padding:"9px 14px", borderBottom:`1px solid rgba(59,130,246,0.05)` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
            ...
            <Table cols={["ASSET","KEY LEN","CIPHER SUITE","TLS","CA"]}>
              {assets.filter(a => a.tls !== "—" && a.tls !== undefined).slice(0, 10).map((a, i) => (
                <TR key={i}>
                  ...
                  <TD style={{ fontSize:10, color:keyColor(a.keylen) }}>{a.keylen}</TD>
                  <TD style={{ fontSize:9, color:T.text3, maxWidth:140, overflow:"hidden",
                    textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{a.cipher}</TD>
