...
export const S = {
    ...
    btn: { background: T.blue3, border: `1px solid rgba(59,130,246,0.32)`, borderRadius: 3, color: T.blue, cursor: "pointer", padding: "5px 12px", fontFamily: "'Orbitron',monospace", fontSize: 8, letterSpacing: "0.12em", whiteSpace: "nowrap" },
...
export function Pulse({ color = "#22c55e" }) {
    return (_jsxs("span", { style: { position: "relative", display: "inline-flex", width: 8, height: 8, flexShrink: 0 }, children: [_jsx("span", { style: { position: "absolute", inset: 0, borderRadius: "50%", background: color, opacity: 0.5, animation: "ping 1.4s ease infinite" } }), _jsx("span", { style: { width: 8, height: 8, borderRadius: "50%", background: color, boxShadow: `0 0 5px ${color}`, display: "block" } })] }));
}
const BADGE_COLORS = {
...
export function Badge({ v, children }) {
    ...
    return (_jsx("span", { style: { display: "inline-flex", alignItems: "center", padding: "2px 8px", borderRadius: 2,
            fontFamily: "'Orbitron',monospace", fontSize: 7.5, letterSpacing: ".08em",
            background: bg, border: `1px solid ${border}`, color: BADGE_TEXT[v] }, children: children }));
}
export function ProgBar({ pct, color }) {
    return (_jsx("div", { style: S.progWrap, children: _jsx("div", { style: { width: `${Math.min(100, pct)}%`, height: "100%", background: color, borderRadius: 2, transition: "width 0.8s ease" } }) }));
}
export function MetricCard({ label, value, sub, color }) {
    return (_jsxs("div", { style: S.metric, children: [_jsx("div", { style: S.lblSm, children: label }), _jsx("div", { style: { ...S.metVal, color, textShadow: `0 0 16px ${color}44` }, children: value }), _jsx("div", { style: S.metSub, children: sub })] }));
}
export function Panel({ children, style }) {
    return _jsx("div", { style: { ...S.panel, ...style }, children: children });
}
export function PanelHeader({ left, right }) {
    return (_jsxs("div", { style: S.ph, children: [_jsx("span", { style: S.lbl, children: left }), right && _jsx("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: right })] }));
}
// ── Shared Table ─────────────────────────────────────────────────────────────
export function Table({ cols, children }) {
    return (_jsx("div", { style: S.tblWrap, children: _jsxs("table", { style: { width: "100%", borderCollapse: "collapse", fontSize: 11 }, children: [_jsx("thead", { children: _jsx("tr", { children: cols.map(c => (_jsx("th", { style: { fontFamily: "'Orbitron',monospace", fontSize: 7, color: T.text3,
                                letterSpacing: ".14em", padding: "7px 12px", borderBottom: `1px solid rgba(59,130,246,0.07)`,
                                textAlign: "left", fontWeight: 400 }, children: c }, c))) }) }), _jsx("tbody", { children: children })] }) }));
}
export function TR({ children, onClick }) {
    return (_jsx("tr", { onClick: onClick, style: { cursor: onClick ? "pointer" : "default" }, onMouseEnter: e => (e.currentTarget.style.background = "rgba(59,130,246,0.04)"), onMouseLeave: e => (e.currentTarget.style.background = "transparent"), children: children }));
}
export function TD({ children, style }) {
    return (_jsx("td", { style: { padding: "8px 12px", borderBottom: `1px solid rgba(59,130,246,0.04)`,
            color: T.text2, fontSize: 11, ...style }, children: children }));
}
// ── Sub-tab bar ──────────────────────────────────────────────────────────────
export function SubTabs({ tabs, active, onChange }) {
    return (_jsx("div", { style: { display: "flex", gap: 2 }, children: tabs.map(t => (_jsx("button", { onClick: () => onChange(t.id), style: { background: "none", border: "none", borderBottom: t.id === active ? "2px solid #3b82f6" : "2px solid transparent",
                padding: "7px 12px", cursor: "pointer", fontFamily: "'Orbitron',monospace", fontSize: 8,
                letterSpacing: ".12em", color: t.id === active ? T.blue : T.text3, transition: "all .2s" }, children: t.label }, t.id))) }));
}
// ── PQC Score Ring ────────────────────────────────────────────────────────────
export function ScoreRing({ score, label, size = 72 }) {
    ...
    return (_jsxs("div", { style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }, children: [_jsxs("svg", { width: size, height: size, viewBox: `0 0 ${size} ${size}`, children: [_jsx("circle", { cx: size / 2, cy: size / 2, r: r, fill: "none", stroke: "rgba(255,255,255,0.05)", strokeWidth: "7" }), _jsx("circle", { cx: size / 2, cy: size / 2, r: r, fill: "none", stroke: color, strokeWidth: "7", strokeDasharray: `${dash} ${circ}`, strokeLinecap: "round", transform: `rotate(-90 ${size / 2} ${size / 2})`, style: { filter: `drop-shadow(0 0 4px ${color})`, transition: "stroke-dasharray 1s ease" } }), _jsx("text", { x: size / 2, y: size / 2 + 5, textAnchor: "middle", fill: color, fontSize: size * 0.2, fontFamily: "Orbitron", fontWeight: "700", children: score })] }), _jsx("span", { style: { fontSize: 8, color: T.text3, letterSpacing: ".08em", fontFamily: "'Share Tech Mono',monospace", textTransform: "uppercase" }, children: label })] }));
}
// ── Shared Mock Data ─────────────────────────────────────────────────────────
