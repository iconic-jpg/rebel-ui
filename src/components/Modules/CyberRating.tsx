...
export default function CyberRatingPage() {
  ...
  useEffect(() => { drawTrend(); }, [trend]);

  function drawTrend() {
    const c = trendRef.current; if (!c || !trend.length) return;
    const ctx = c.getContext("2d")!;
    const W = c.offsetWidth || 800, H = 180;
    c.width = W;
  ...
  return (
    <div style={S.page}>
      ...
      <Panel style={{ background:"linear-gradient(135deg,rgba(59,130,246,0.06) 0%,transparent 100%)" }}>
        <div style={{ padding:"20px 24px", display:"flex", gap:32, alignItems:"center", flexWrap:"wrap" }}>
          {/* Big score */}
          <div>
            ...
            <div style={{ display:"flex", alignItems:"baseline", gap:10 }}>
              <span style={{ fontFamily:"'Orbitron',monospace", fontSize:64, fontWeight:900, color:rColor, textShadow:`0 0 30px ${rColor}66`, lineHeight:1 }}>
                {rating}
              </span>
              <span style={{ fontFamily:"'Orbitron',monospace", fontSize:20, color:T.text2 }}>/100</span>
          ...
          <svg width="140" height="140" viewBox="0 0 140 140">
            ...
            <text x="70" y="66" textAnchor="middle" fill={rColor} fontFamily="Orbitron,monospace" fontSize="20" fontWeight="700">{rating}</text>
            <text x="70" y="83" textAnchor="middle" fill="rgba(200,220,255,0.35)" fontFamily="Share Tech Mono" fontSize="8">{rLabel}</text>
          </svg>
        </div>
      </Panel>
      ...
      <Panel>
            ...
            <div style={{ display:"flex", gap:12 }}>
              {[["AVG RISK", T.red], ["SCANS", T.blue]].map(([l, c]) => (
                <div key={l} style={{ display:"flex", alignItems:"center", gap:4 }}>
                  <div style={{ width:18, height:2, background:c as string, boxShadow:`0 0 4px ${c}` }} />
        ...
        <div style={{ padding:"10px 4px 4px" }}>
          {loading ? (
            <div style={{ height:180, display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, color:T.text3 }}>
              Loading trend data...
            </div>
          ) : (
        ...
        <div style={{ padding:14, display:"flex", flexDirection:"column", gap:10 }}>
          {[
            ...
            <div key={row.label} style={{ display:"flex", alignItems:"center", gap:12 }}>
              ...
              <span style={{ flex:1, fontSize:11, color:T.text2 }}>{row.label}</span>
              <div style={{ width:120 }}>
                <ProgBar pct={total ? Math.round(row.val / total * 100) : 0} color={row.color} />
              </div>
              <span style={{ fontFamily:"'Orbitron',monospace", fontSize:12, color:row.color, minWidth:32, textAlign:"right" }}>
                {row.val}
              </span>
            </div>
          ))}
