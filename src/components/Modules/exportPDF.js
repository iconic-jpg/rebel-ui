...
function keyCol(kl) {
    if (!kl)
        return "#64748b";
    if (kl.startsWith("1024"))
        return "#dc2626";
    if (kl.startsWith("2048"))
        return "#d97706";
    return "#16a34a";
}
function tlsCol(tls) {
...
export function buildAuditHTML(opts) {
    const { enriched, migrationScore, total, totalDays, calDays, totalCostINR, teamSize, devRateINR, completionDate, clientName, clientDomain, milestones, } = opts;
    ...
    const doraRows = [
        ...
        { art: "Art. 9.4", title: "Cryptographic Controls", desc: "Implement controls protecting data in transit and at rest to current industry standards.", status: enriched.filter((a) => a.status !== "weak" && a.status !== "WEAK").length > 0 ? "PARTIAL" : "PENDING" },
    ...
    const assetRows = enriched.map((a, i) => {
        ...
        const dots = ps
            ? Object.values(ps.criteria).map((c) => `<span class="crit-dot" style="background:${c.pass ? "#16a34a" : c.pts > 0 ? "#d97706" : "#dc2626"};"></span>`).join("")
            : "";
        return `<tr>
    ...
    // ── Remediation cards ────────────────────────────────────────────────────────
    const remHTML = [
        { title: "Upgrade Certificate Key", color: "#dc2626", fix: "Deploy RSA-4096 or EC P-384 certificates. Primary gate — worth 70/100 pts.", items: enriched.filter((a) => !(a.pqcScore?.criteria?.certKey4096?.pass)).map((a) => a.app) },
        { title: "Remove Wildcard Certificates", color: "#ea580c", fix: "Replace wildcards with dedicated per-service certificates. Worth 20/100 pts.", items: enriched.filter((a) => a.is_wildcard).map((a) => a.app) },
        { title: "Deploy Kyber Hybrid KX", color: "#1d4ed8", fix: "Implement X25519+CRYSTALS-Kyber768 (NIST FIPS 203). Required for ACTIVE status.", items: enriched.filter((a) => !a.pqc).map((a) => a.app) },
    ].map(s => {
        const rows = s.items.length > 0
            ? s.items.slice(0, 6).map(app => `<div class="rem-item"><span style="color:${s.color};flex-shrink:0;font-size:8pt;">▸</span>${app}</div>`).join("") +
