...
export function parseCipher(cipher, kxGroup) {
    ...
    if (c.includes("AES_256_GCM") || c.includes("AES256-GCM"))
        ...
        bulk = "AES-128-CCM";
    else if (c.includes("AES_256_CBC") || c.includes("AES256-CBC") || (c.includes("AES256") && !c.includes("GCM") && !c.includes("CCM")))
        bulk = "AES-256-CBC";
    else if (c.includes("AES_128_CBC") || c.includes("AES128-CBC") || (c.includes("AES128") && !c.includes("GCM") && !c.includes("CCM")))
        bulk = "AES-128-CBC";
...
// ── Finding generator ─────────────────────────────────────────────────────────
export function analyseCipher(components, tlsNormalised // already normalised — "1.3", "1.2", "1.1", "1.0"
) {
    const findings = [];
