...
export default function CBOMPage() {
  ...
  useEffect(() => { drawKeyLength(); }, [keyData, mobile]);
  ...
  function drawKeyLength() {
    ...
    const bw = mobile ? 24 : 30, gap = mobile ? 10 : 16;
  ...
  return (
    <div style={S.page}>
      ...
      {/* CHARTS ROW */}
      <div style={{ display:"grid", gridTemplateColumns: mobile ? "1fr" : "repeat(3,1fr)", gap: mobile ? 8 : 10 }}>
            ...
            {cipherData.map(c => (
              <div key={c.name}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                  <span style={{ fontSize:9, color:T.text3, overflow:"hidden", textOverflow:"ellipsis",
                    whiteSpace:"nowrap", maxWidth: mobile ? 140 : 160 }}>{c.name}</span>
                  <span style={{ fontSize:9, fontFamily:"'Orbitron',monospace", color:c.color,
                    flexShrink:0, marginLeft:8 }}>{c.count}</span>
                </div>
        ...
        <Panel>
          ...
          <div style={{ padding:14, display:"flex", gap:mobile?12:0,
            flexDirection: mobile ? "row" : "column", alignItems: mobile ? "center" : "stretch" }}>
            <canvas ref={caRef} width={160} height={160}
              style={{ display:"block", margin: mobile ? "0" : "0 auto 10px",
              flexShrink:0, width: mobile ? 100 : 160, height: mobile ? 100 : 160 }} />
            ...
            {displayData.map((d: any, i: number) => (
              <div key={i} style={{ padding:"10px 14px", borderBottom:`1px solid rgba(59,130,246,0.05)` }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  ...
                  <Badge v={d.status==="ok"?"green":"red"}>{d.status==="ok"?"OK":"WEAK"}</Badge>
                </div>
                <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:3 }}>
                  <Badge v={d.tls==="1.0"?"red":d.tls==="1.2"?"yellow":"green"}>TLS {d.tls}</Badge>
                  <span style={{ fontSize:9, color: d.keylen?.startsWith("1024") ? T.red : d.keylen?.startsWith("2048") ? T.yellow : T.green }}>
                    {d.keylen}
                  </span>
                  <span style={{ fontSize:14, color: d.pqc ? T.green : T.red }}>{d.pqc ? "✓" : "✗"}</span>
                </div>
      ...
      <Panel>
        ...
        <div style={{ padding:14, display:"flex", gap:16, alignItems:"center",
          ...
          <canvas ref={protoRef} width={140} height={140}
            style={{ width: mobile ? "100%" : 140, height:140, maxWidth:200 }} />
          <div ref={protoLegendRef} style={{ display:"flex", flexDirection:"column",
            gap:9, flex:1, width: mobile ? "100%" : "auto" }} />
        </div>
      </Panel>
    </div>
  );
