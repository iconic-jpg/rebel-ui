...
export default function ReportingPage() {
  ...
  function drawActivityChart() {
    ...
    ctx.font = `${mobile ? 6 : 7}px 'Share Tech Mono'`; ctx.textAlign = "center";
    trend.forEach((t, i) => {
      if (i % (mobile ? 3 : 2) === 0) {
  ...
  return (
    <div style={S.page}>
      ...
      {/* HEADER */}
      <Panel>
        <div style={{ padding: mobile ? "12px 14px" : "16px 20px",
          display:"flex", justifyContent:"space-between", alignItems: mobile ? "flex-start" : "center",
          flexDirection: mobile ? "column" : "row", gap: mobile ? 10 : 0 }}>
          <div>
            <div style={{ fontFamily:"'Orbitron',monospace", fontSize:9, color:T.text3,
              letterSpacing:".15em", marginBottom:4 }}>
              REBEL THREAT INTELLIGENCE
            </div>
            <div style={{ fontFamily:"'Orbitron',monospace", fontSize: mobile ? 15 : 18,
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button style={{ ...S.btn, color:T.green, borderColor:`${T.green}44` }}>↓ PDF</button>
          </div>
        </div>
      </Panel>
      ...
      <Panel>
        ...
        <div style={{ padding:"10px 4px 4px" }}>
          {loading ? (
            <div style={{ height: mobile ? 160 : 200, display:"flex", alignItems:"center",
        ...
        <div style={{ padding:14, display:"flex", flexDirection:"column", gap:10 }}>
          ...
          {users.map((u: any, i: number) => (
            <div key={i} style={{ display:"flex", alignItems:"center", gap: mobile ? 8 : 12 }}>
              ...
              <span style={{ flex:1, fontSize: mobile ? 10 : 11, color:T.text2,
                fontFamily:"Share Tech Mono", overflow:"hidden", textOverflow:"ellipsis",
                whiteSpace:"nowrap", minWidth:0 }}>{u.user}</span>
              <div style={{ width: mobile ? 60 : 120, flexShrink:0 }}>
                <ProgBar pct={Math.round(u.scans / maxUser * 100)} color={T.blue} />
              </div>
              <span style={{ fontFamily:"'Orbitron',monospace", fontSize: mobile ? 10 : 11,
                color:T.blue, minWidth: mobile ? 30 : 40, textAlign:"right" }}>
                {u.scans}
              </span>
              {!mobile && (
      ...
      <Panel>
        ...
        <div style={{ padding:14, display:"flex", flexDirection:"column", gap: mobile ? 8 : 12 }}>
          {[
            ...
            <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
              ...
              background:"rgba(255,255,255,0.02)", border:`1px solid rgba(59,130,246,0.06)`, borderRadius:2 }}>
              <span style={{ fontSize: mobile ? 10 : 11, color:T.text2 }}>{row.label}</span>
              <span style={{ fontFamily:"'Orbitron',monospace", fontSize: mobile ? 11 : 12,
